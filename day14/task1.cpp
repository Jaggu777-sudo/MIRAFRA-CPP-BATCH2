...
class BST{
	public:
		...
		struct tree{
			...
			tree(error val):data(val){}
		};
		unique_ptr<tree>root;
		void inorder(){
			inorder_impl(root.get());
		}
		void delete_node(error val){
			delete_node_impl(root,val);
		}
		void insert(error val){
	...
	private:
		...
		void delete_node_impl(unique_ptr<tree>&node,error val){
			if(!node){
				return;
			}
			if(val<node->data){
		...
		void insert_impl(unique_ptr<tree>&root,error val){
			if(!root){
...
int main(){									
	BST obj;
	obj.insert(BST::Wiper);
	obj.insert(BST::AirBag);
	obj.insert(BST::Engine);
	obj.insert(BST::Dash_Board);
	obj.insert(BST::Fuel);
	//cout<<"Before delete 30"<<endl;
	obj.inorder();
	cout<<endl;
	cout<<"after deleting wiper error id"<<endl;
	obj.delete_node(BST::Wiper);
	obj.inorder();
	cout<<endl;
	//cout<<"search ele(100):"<<endl;
	cout<<boolalpha<<obj.search(BST::Engine)<<endl;
